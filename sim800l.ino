...
void loop() {
  ...
  String RSMS;             //We add this new variable String type, and we put it in loop so everytime gets initialized
    ...
    while(sim800l.available()>0){       //When SIM800L sends something to the Arduino... problably the SMS received... if something else it's not a problem
        ...
        DHT_OK=RSMS.indexOf("BLF184OFF");   //And this is why we changed from char to String, it's to be able to use this function "indexOf"
        MLX_OK=RSMS.indexOf("BLF184ON");   //"indexOf function looks for the substring "x" within the String (here RSMS) and gives us its index or position
  ...
  if(DHT_OK!=-1){                         //If "DHT" word is found within the SMS, it means that DHT_OK have other value than -1 so we can proceed
    Serial.println("found OFF");          //Shows on the serial monitor "found DHT" (optional)
    delay(100);
    digitalWrite(4, HIGH);
    Serial.print("LED APAGADO");
                                         //Show it on the serial monitor also optional

    Data_SMS = "RELE APAGADO";       //Prepare the SMS to send, it contains some strings like "DHT" "Temperature"...
  ...
  delay(1000);
  Serial.println("found ONN");
  delay(100);
  digitalWrite(4, LOW);
  Serial.print("LED PRENDIDO ");

  Data_SMS = "RELE ENCENDIDO";
  EEPROM.put(0,false);
  Send_Data();
  ReceiveMode();
...
void Send_Data()
{
  ...
  delay(100);
  sim800l.print("AT+CMGS=\"+51984773621\"\r");  //Your phone number don't forget to include your country code example +212xxxxxxxxx" --> +51984594009
  delay(500);  
